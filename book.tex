...
notification.sendEmail(order.email, "order\_confirmation")
...
addToCart() görmeliyiz. “User inserts item into shopping\_basket” gibi farklı terimler kullan-
...
metrics.record("payment\_call", success=false)
...
id & customer\_id & status & total & updated\_at \\
...
event\_id & aggregate\_id & event\_type & event\_data & occurred\_at \\
...
// SNAPSHOT TABLE
\end{lstlisting}
TABLE order\_snapshots:

aggregate\_id & version & state\_data & created\_at \\
...
email = get\_customer\_email(customer\_id)
...
’Body’: {’Text’: {’Data’: format\_order\_email(order)}}
...
availability\_zone = data.aws\_availability\_zones.available.names[
...
KUBECONFIG: ${{ secrets.STAGING\_KUBECONFIG }}
...
KUBECONFIG: ${{ secrets.PROD\_KUBECONFIG }}
...
\textit{"{natural\_language\_request}"}

Return JSON with: product\_id, quantity, shipping\_address
...
score = exact\_match(response, test.expected)

ELSE IF test.expected\_contains:

score = contains\_all(response, test.expected\_contains)

ELSE IF test.rubric:

score = llm\_judge(response, test.rubric)
...
RETURN aggregate\_scores(results)
...
{"correctness": X, "readability": X, "efficiency": X, "best\_practices
...
judgment = claude\_opus.generate(prompt)
...
regressions = []
\end{lstlisting}
FOR test IN test\_cases:

old\_result = model.generate(old\_prompt, test.input)

new\_result = model.generate(new\_prompt, test.input)

old\_score = evaluate(old\_result, test)

new\_score = evaluate(new\_result, test)

old\_scores.append(old\_score)

new\_scores.append(new\_score)

IF new\_score < old\_score - THRESHOLD:

regressions.append({

\textit{"test": test,}

\textit{"old\_score": old\_score,}

\textit{"new\_score": new\_score,}

\textit{"delta": new\_score - old\_score}

})

RETURN {

\textit{"old\_avg": mean(old\_scores),}

\textit{"new\_avg": mean(new\_scores),}

\textit{"regressions": regressions,}

\textit{"can\_deploy": len(regressions) == 0}
...
generate\_tests\_for\_lines(file, lines)
...
Available tools: search, calculator, code\_executor

Think step by step.

Respond with: THOUGHT, ACTION, or FINAL\_ANSWER
...
tool\_name = thought.tool

tool\_input = thought.input

observation = execute\_tool(tool\_name, tool\_input)
...
result = execute\_tool(call.name, call.args)

results.append(ToolMessage(content=result))

RETURN {"messages": results, "tools\_used": [c.name FOR c IN
...
graph.add\_node(entity.name, type=entity.type)

FOR rel IN relations:

graph.add\_edge(rel.source, rel.target, type=rel.relation)
...
graph.add\_community\_summary(community.id, summary)
...
Respond: RELEVANT or NOT\_RELEVANT
...
\end{lstlisting}
IF log\_entry.risk\_score > THRESHOLD:

alert\_security\_team(log\_entry)
...
display\_to\_human(step)

approval = wait\_for\_human\_approval()

IF approval.rejected:

RETURN handle\_rejection(step, approval.reason)

result = execute\_step(step)

ELSE IF risk\_level == "medium":
